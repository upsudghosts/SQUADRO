...
public class BoardSquadro extends ABoard<MoveSquadro, RoleSquadro, BoardSquadro> {
	...
	/** Methode créant un board a partir d'un ArrayList de points */
	...
	/** Methode renvoyant un ArrayList contenant les pieces adverse se trouvant a la suite en fonction de qui a joué et quelle piece a ete bougee*/
	...
	/** Methode permettant de jouer un coup, renvoyant le nouveau board apres l'avoir joué*/
	@Override
		...
		if (AR == 0) {//pour l'aller
			for (int i = 0; i < speed; i++) {//on fait le nombre de pas necessaire (== speed)
				...
				if (nbAdv == 0) {//si aucun adversaire
					...
					break;//si on a sauté des adversaires, on n'avance plus
				}
